...
const SPOTIFY_AUTHORIZATION = Buffer.from(`${env.SPOTIFY_CLIENT_ID}:${env.SPOTIFY_CLIENT_SECRET}`).toString('base64');
...
export function createProtectedRouter() {
  return createRouter().middleware(({ ctx, next }) => {
    ...
    if (!jwt) {
      throw new trpc.TRPCError({ code: "UNAUTHORIZED", message: "No JWT found" });
    }
